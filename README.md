...
> **Стадия проекта:** веб-MVP. В коде есть поиск через API hh.ru, AI-разбор и черновики через OpenRouter, онбординг, кабинет и трекер. Автоотклик работает только после отдельного подключения аккаунта hh.ru и включения агентного режима; развёртывание требует ключей, OAuth redirect URI и миграций.
